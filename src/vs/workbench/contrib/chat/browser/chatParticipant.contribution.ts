...
// dsh: no Chat view container. The conversation lives in the DSH web app
// beside the workbench, so the panel would only duplicate it; the participant
// extension point below (and the inline chat it feeds) stays fully wired.
// Everything else that references ChatViewId degrades: openView returns
// undefined and the panel-only flows are unreachable without the view.
